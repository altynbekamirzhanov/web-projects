...
function process(responses) {
	...
	if (category == "new_books") {
		...
		document.getElementById("p_main").style.display = "block";
		
		switch(category) {
			case "childrens": list = array.childrens; break;
			case "science": list = array.science; break;
			case "detective": list = array.detective; break;
			case "romance": list = array.romance; break;
			case "poetry": list = array.poetry;
		}
	}
	
	categoryName = list['title'];
	document.getElementById('title').textContent = list['title'];
	document.getElementById('first_title').textContent = list['category'];
	document.getElementById('second_title').textContent = list['category'];
	document.getElementById('category_image').src = "Images/Category/"+list['directory']+"/category_default.jpg";
	
	let subcategories = list['subcategories'];
	for (i=1; i<=subcategories.length; i++) {
		document.getElementById('subcategory' + i).src = "Images/Category/"+list['directory']+"/category"+i+".jpg";
		document.getElementById('subcategory_' + i).textContent = subcategories[i-1];
	}
}
...
function call(event){
	...
	category = element.id;
	fetch('http://demo1373984.mockable.io/list_page').then(onSuccess,onFail);
}

document.getElementById('new_books').addEventListener('click', call);
document.getElementById('childrens').addEventListener('click', call);
document.getElementById('science').addEventListener('click', call);
document.getElementById('detective').addEventListener('click', call);
document.getElementById('romance').addEventListener('click', call);
document.getElementById('poetry').addEventListener('click', call);
...
let list = [{"category":"HUMOR-SATIRE", "name": "Penguin Classics Penguin's Poems for Love", "description": "Well, reading books as a hobby was ..."}];